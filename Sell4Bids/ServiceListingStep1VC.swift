...
class ServiceListingStep1VC: UIViewController,UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate, UITextFieldDelegate{
    ...
    @IBOutlet weak var ServiceBuyerAndBuyerView: UIView!
    @IBOutlet var borderLbl: UILabel!
    
    
    var selectbtn = String()
    var limitcount = 0
    ...
    //TakeSellerBuyerCustomVariable
    @IBOutlet weak var OfferSwitch: UISwitch!
    @IBOutlet weak var WantSwitch: UISwitch!
    @IBOutlet weak var SwitchView: UIView!
    
    //TakeTitleCustomVariable
    @IBOutlet weak var ServiceTitleLbl: UILabel!
    @IBOutlet weak var ServiceTitleTextField: UITextField!
    ...
    @IBAction func CloseActionBtn(_ sender: Any) {
        ...
        if self.imageList.count <= 0 {
            ...
            self.TakeItemconstant.constant = 45
        ...
        bs_presentImagePickerController(vc, animated: true,
            ...
            DispatchQueue.main.async {
                ...
                if self.imageList.count > 0 {
                    ...
                    self.TakePhotoView.isHidden = true
                    self.ImageCustomViewHeight.constant = 200
                    self.TopConstraintValue.constant = 55
                    self.Scrollview.layoutIfNeeded()
                    self.CollectionViewHeight.constant = 80
    ...
    @objc func showExtraImgView () {
        if imageList.count >= 14 {
            showSwiftMessageWithParams(theme: .error, title: "Photo Max Limit Error", body: "You cannot add more than 14 photos.")
        }else {
    ...
    @objc func offerSwitchTapped (sender:UISwitch){
        if sender.isOn == true {
            WantSwitch.isOn = false
            selectbtn = "Offer"
            print("Selected_Button == \(selectbtn)")
        }else {
            WantSwitch.isOn = true
            selectbtn = "Offer"
            print("Selected_Button == \(selectbtn)")
        }
    }
    
    @objc func wantedSwitchTapped (sender:UISwitch){
        if sender.isOn == true {
            OfferSwitch.isOn = false
            selectbtn = "Want"
            print("Selected_Button == \(selectbtn)")
        }else {
            OfferSwitch.isOn = true
            selectbtn = "Want"
            print("Selected_Button == \(selectbtn)")
        }
    }
    ...
    override func viewDidLoad() {
        ...
        SwitchView.makeCornersRound()
        SwitchView.layer.cornerRadius = 6.0
        
//        ServiceTitleTextField.makeCornersRound()
//        ServiceTitleTextField.layer.borderWidth = 1.0
        
        OfferSwitch.addTarget(self, action: #selector(offerSwitchTapped(sender:)), for: .allTouchEvents)
        
        WantSwitch.addTarget(self, action: #selector(wantedSwitchTapped(sender:)), for: .allTouchEvents)
        ...
        ServiceBuyerAndBuyerView.addShadow()
        
        
        
        ServiceTitleView.addShadow()
        ...
        ServiceTitleTextField.endEditing(true)
...
extension ServiceListingStep1VC : getImageDelegate {
