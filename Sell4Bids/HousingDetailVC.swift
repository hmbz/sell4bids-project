...
class HousingDetailVC: UIViewController,GMSMapViewDelegate,UIGestureRecognizerDelegate {
    ...
    @IBOutlet weak var realEstateCategoryLbl: UILabel!
    @IBOutlet weak var realEstateTypeLbl: UILabel!
    @IBOutlet weak var bedroomsLbl: UILabel!
    @IBOutlet weak var bathroomsLbl: UILabel!
    @IBOutlet weak var laundryLbl: UILabel!
    @IBOutlet weak var parkingLbl: UILabel!
    @IBOutlet weak var squareFeetLbl: UILabel!
    @IBOutlet weak var availableOnLbl: UILabel!
    @IBOutlet weak var openHouseDateLbl: UILabel!
    @IBOutlet weak var userViewHeight: NSLayoutConstraint!
    @IBOutlet weak var rightArrowImg: UIImageView!
    @IBOutlet weak var openHouseFirst: UILabel!
    ...
    var selectedProduct : housingDetailModel?
    ...
    private func showDataOnView() {
        ...
        self.priceLbl.text = "\(selectedProduct?.currencySymbol ?? "") \(selectedProduct?.startPrice ?? 0)"
        ...
        self.realEstateCategoryLbl.text = selectedProduct?.housingCategory
        self.realEstateTypeLbl.text = selectedProduct?.housingType
        self.bedroomsLbl.text = "\(selectedProduct?.bedrooms ?? 0)"
        self.bathroomsLbl.text = "\(selectedProduct?.bathrooms ?? 0)"
        self.laundryLbl.text = selectedProduct?.laundry
        self.parkingLbl.text = selectedProduct?.parking
        self.squareFeetLbl.text = "\(selectedProduct?.squareFeet ?? 0)"
        let availbleHouseDate = convertTimeStampToDate(timeStamp: selectedProduct?.availableOn ?? 0)
      var timeArr = [String]()
      var values = [String]()
      
      for a in (selectedProduct?.openHouseDate)!{
        let b = String(describing: a)
         values = b.components(separatedBy: ",")
        if values.count > 0 {
          for b in values{
            let toint = Int64(b)
            if toint != nil || toint != 0{
              let timetodate = convertTimeStampToDate(timeStamp: toint)
              if timetodate == "01-01-1970"{
                
              }
                else{
                 timeArr.append(timetodate)
              }
...
//      let time = selectedProduct?.openHouseDate?.last as? Int64
//      let timetodate = convertTimeStampToDate(timeStamp: time)
      if timeArr.count > 0{
        openHouseFirst.isHidden = false
      }
      self.openHouseDateLbl.text = timeArr.joined(separator: ",")
        self.availableOnLbl.text = availbleHouseDate
        self.addressLbl.text = address
        self.mapAddressLbl.text = address
        self.pageControl.numberOfPages = selectedProduct?.imagesPath?.count ?? 0
...
//              if selectedProduct?.isExpiredNotified == true{
...
//                    self.listedEndedLblHeight.constant = 0
    ...
    private func convertTimeStampToDate(timeStamp:Int64?) -> String {
        if timeStamp != 0 {
            let date = Date(timeIntervalSince1970: TimeInterval((timeStamp ?? 0) / 1000))
            let dateFormatter = DateFormatter() //Set timezone that you want
            dateFormatter.locale = NSLocale.current
            dateFormatter.dateFormat = "dd-MM-yyyy" //Specify your format that you want
            let strDate = dateFormatter.string(from: date)
            print("Availble Date =",strDate)
            return strDate
        }else {
            ...
            Networking.instance.postApiCall(url: itemDetailUrl, param: parameter) { (response, error, statusCode) in
                ...
                if status {
                    ...
                    for item in msgArray {
                        ...
                        let quantity = msgDic["quantity"]?.int ?? 1
                        let bedrooms = msgDic["bedrooms"]?.int ?? 0
                        let noSmoking = msgDic["noSmoking"]?.bool ??  false
                        let wheelChairAccess = msgDic["wheelChairAccess"]?.bool ?? false
                        let description = msgDic["description"]?.string ?? ""
                        let parking = msgDic["parking"]?.string ?? ""
                        let petsAccepted = msgDic["petsAccepted"]?.bool ??  false
                        let itemCategory = msgDic["itemCategory"]?.string ?? ""
                        let housingType = msgDic["housingType"]?.string ?? ""
                        let availableOn = msgDic["availableOn"]?.int64 ?? 0
                        let openHouseDate = msgDic["openHouseDate"]?.array ?? []
                        let bathrooms = msgDic["bathrooms"]?.int ?? 0
                        let housingCategory = msgDic["housingCategory"]?.string ?? ""
                        let furnished = msgDic["furnished"]?.bool ?? false
                        let laundry = msgDic["laundry"]?.string ?? ""
                        let squareFeet = msgDic["squareFeet"]?.double ?? 0
                        let imagesPath = msgDic["images_path"]?.array ?? []
                        ...
                        let isExpiredNotified = msgDic["isExpiredNotified"]?.bool ?? false
                        let autoListingCount = msgDic["autoListingCount"]?.int ?? 0
                        ...
                        // For Order and Offer Conditions
                        ...
                        self.selectedProduct = housingDetailModel.init(watchlistStatus: watchlistStatus, sellerIsBlocked: sellerIsBlocked, status: status, unauthorized: unauthorized, uid: uid, itemId: itemId, startTime: startTime, chargeTime: chargeTime, endTime: endTime, title: title, startPrice: startPrice, quantity: quantity, description: description, itemCategory: itemCategory, imagesPath: imagesPath,imagePathSmall: imagePathSmall, bedrooms: bedrooms, noSmoking: noSmoking, wheelChairAccess: wheelChairAccess, parking: parking, petsAccepted: petsAccepted, housingType: housingType,availableOn: availableOn, openHouseDate: openHouseDate, bathrooms: bathrooms, housingCategory: housingCategory, furnished: furnished, laundry: laundry, squareFeet: squareFeet, currencyString: currencyString, currencySymbol: currencySymbol, state: state, longitude: longitude, latitude: latitude, countryCode: countryCode, zipCode: zipCode, city: city, itemAuctionType: itemAuctionType, visibility: visibility, isExpiredNotified: isExpiredNotified, autoListingCount: autoListingCount, acceptOffers: acceptOffers, ordering: ordering, listingEnded: listingEnded, autoReList: autoReList, adminVerify: adminVerify, platform: platform, offerArray: self.offerArray, orderArray: self.orderArray)
                ...
                if error?.contains("The network connection was lost") ?? false{
                ...
                if error?.contains("The Internet connection appears to be offline.") ?? false {
                    let alert = UIAlertController.init(title: "Network Error".localizableString(loc: LanguageChangeCode), message: "Opps Network Error".localizableString(loc: LanguageChangeCode), preferredStyle: .alert)
                    let ok = UIAlertAction.init(title: "Ok".localizableString(loc: LanguageChangeCode), style: .cancel, handler: nil)
                    alert.addAction(ok)
                    self.present(alert, animated: true, completion: nil)
                ...
                if error?.contains("A server with the specified hostname could not be found.") ?? false{
                    let alert = UIAlertController.init(title: "Network Error".localizableString(loc: LanguageChangeCode), message: "Opps Network Error".localizableString(loc: LanguageChangeCode), preferredStyle: .alert)
                    let ok = UIAlertAction.init(title: "Ok".localizableString(loc: LanguageChangeCode), style: .cancel, handler: nil)
                    alert.addAction(ok)
                    self.present(alert, animated: true, completion: nil)
    ...
    private func showCounterOfferView(){
        ...
        let currencySymbol = selectedProduct?.currencySymbol ?? ""
    ...
    @objc func updateTimer(currentTime: NSNumber?){
        ...
        if timeRemaining <= 0 || timeRemaining == -1{
            ...
            self.remainingTimeViewHeight.constant = 40
    ...
    @objc func viewBidsBtnTapped(sender: UIButton) {
        ...
        vc.itemId = self.selectedProduct?.itemId ?? ""
        vc.currencySymbol = self.selectedProduct?.currencySymbol ?? ""
        vc.modalPresentationStyle = .fullScreen
        let navController = UINavigationController(rootViewController: vc)
        self.present(navController, animated: true, completion: nil)
    }
    ...
    @objc func bidNowBtnTapped(sender: UIButton) {
        ...
        if SessionManager.shared.isUserLoggedIn == true {
            ...
            let country = selectedProduct?.countryCode?.uppercased()
            let currencySymbol = CurrencyManager.instance.getCurrencySymbol(Country: country ?? "USA")
            BidNowCustomView.textFieldTitleLbl.text =  "Bid price must be greater than \(currencySymbol)\(selectedProduct?.startPrice ?? 0)"
    ...
    @objc func bidNowApiCall() {
        if BidNowCustomView.firstTextField.text!.isEmpty {
            self.view.makeToast("Price can't be empty", position:.top)
        }else {
    ...
    @objc func offerNowBtnTapped(sender: UIButton) {
        ...
        if SessionManager.shared.isUserLoggedIn == true {
            OfferNowCustomView.titleLbl.text = "Offer Now"
            OfferNowCustomView.textFieldTitleLbl.text = "Enter Offer Price"
            let currencySymbol = selectedProduct?.currencySymbol
            OfferNowCustomView.firstTextField.placeholder = currencySymbol
...
//            let uid = self.selectedProduct?.uid ?? ""
        ...
        if SessionManager.shared.isUserLoggedIn == true {
            ...
            Networking.instance.postApiCall(url: getChatUrl, param: parameter) { (response, Error, StatusCode) in
                ...
                if jsonStatus == true {
                    ...
                    //                SweetAlert().showAlert("StrChat".localizableString(loc: LanguageChangeCode), subTitle: MsgString, style: AlertStyle.warning, buttonTitle: StrYes, buttonColor: UIColor.black, otherButtonTitle: StrNo, otherButtonColor: UIColor.black) { (ifYes) -> Void in
                    //                    if(ifYes == true){
                    //                        self.ChatMessage.removeAll()
    ...
    @objc func reListItemBtnTapped(sender: UIButton){
        ...
        let symbol = selectedProduct?.currencySymbol ?? ""
                ...
                if(ifYes == true){
                    ...
                    Networking.instance.postApiCall(url: itemRelistUrl, param: parameter) { (response, Error, StatusCode) in
                        ...
                        if jsonStatus == true {
                            ...
                            self.relistItemCustomView.priceTextField.text = nil
                            self.relistItemCustomView.Quantitytxt.text = nil
                            self.reloadData()
                        }else {
    ...
    @objc func counterOfferBtnTapped(sender: UIButton){
        ...
        self.SendCounterOffer.Quantitytxt.isHidden = true
        self.SendCounterOffer.Header1_Title.isHidden = true
...
extension HousingDetailVC: UITextFieldDelegate{
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if  textField == BidNowCustomView.firstTextField || textField == OfferNowCustomView.firstTextField{
...
extension HousingDetailVC : UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == imageCollectionView{
            ...
            let encodeImage = image.replacingOccurrences(of: " ", with: "%20")
            cell.userImage.sd_setImage(with: URL(string: encodeImage), placeholderImage: #imageLiteral(resourceName: "Profile-image-for-sell4bids-App-1"))
...
extension HousingDetailVC: UIPickerViewDataSource, UIPickerViewDelegate{
