...
class Services_ViewOffers: UIViewController , UITableViewDelegate , UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.OfferArray[indexPath.row].status == "Pending" {
            ...
            cell.priceLbl.text = "\(self.OfferArray[indexPath.row].boughtPrice) X \(self.OfferArray[indexPath.row].boughtQuantity)"
            cell.itemQuantity.text = "Pending"
            cell.itemQuantity.textColor = colorBlue
            ...
            cell.itemQuantity.textColor = colorGreen
            ...
            cell.itemQuantity.textColor = colorRed
    ...
    func ReloadOfferData() {
        ...
        print("selectedProduct!.item_id ==\(self.selectedProduct!.item_id)")
        print("SessionManager.shared.image == \(SessionManager.shared.image)")
        ...
        MainApi.Get_Offers(seller_uid: SessionManager.shared.userId, item_id: selectedProduct!.itemKey, type: "offers", sellerImage: SessionManager.shared.image) { (status, data, error) in
            ...
            if status {
                ...
                if message.contains("No Offers received for this service") {
                   
                  _ = SweetAlert().showAlert("Offers", subTitle: message, style: .error,buttonTitle:"Ok".localizableString(loc: LanguageChangeCode), buttonColor:UIColor.colorFromRGB(0x000000))
