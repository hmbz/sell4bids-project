...
class ViewOrders_Vehicles: UIViewController ,UITableViewDelegate , UITableViewDataSource, UIGestureRecognizerDelegate {
    ...
    public var ServiceDetails : sellingModel
    public var VehiclesDetails: sellingModel
    
    //MARK:- View life cycle
    
    func ReloadOrderData() {
        ordersArray.removeAll()
        MainApi.Get_Orders(seller_uid: SessionManager.shared.userId, item_id: self.productDetail!.id, type: "orders", sellerImage: "") { (status, data, error) in
