...
class JobListingPostVC: UIViewController {
    
    //MARK:- Outlet and Properties
    @IBOutlet weak var currencySymbolBtn: UIButton!
    @IBOutlet weak var priceTextField: UITextField!
    @IBOutlet weak var priceBorderLine: UILabel!
    @IBOutlet weak var payPeriodTextField: UITextField!
    @IBOutlet weak var listIndefinitlySwitch: UISwitch!
    ...
    @IBOutlet weak var automatcRelistingView: UIStackView!
    @IBOutlet weak var automaticRelistingSwitch: UISwitch!
    @IBOutlet weak var automaticRelistingBorderLine: UILabel!
    @IBOutlet weak var listingDurationHeight: NSLayoutConstraint!
    @IBOutlet weak var listingDurationBorderHeight: NSLayoutConstraint!
    @IBOutlet weak var automaticRelistingHeight: NSLayoutConstraint!
    @IBOutlet weak var automaticRelistingBorderHeight: NSLayoutConstraint!
    ...
    let payPeriodArray = ["Select Category","Daily", "BiWeekly", "Weekly", "Monthly","Project based"]
    ...
    @objc func listIndefinitelyBtnTapped(sender: UISwitch) {
        if sender.isOn {
            listingDurationHeight.constant = 0
            listingDurationBorderLine.isHidden = true
            automaticRelistingHeight.constant = 0
            automaticRelistingBorderLine.isHidden = true
            automatcRelistingView.isHidden = true
            listingDurationValue = ""
            listingDurationTextField.placeholder = "3 Days"
            listingDurationBorderHeight.constant = 0
            automaticRelistingBorderHeight.constant = 0
        }else {
            listingDurationHeight.constant = 35
            listingDurationBorderLine.isHidden = false
            automaticRelistingHeight.constant = 35
            automaticRelistingBorderLine.isHidden = false
            automatcRelistingView.isHidden = false
            listingDurationBorderHeight.constant = 1
            automaticRelistingBorderHeight.constant = 1
            automaticRelistingSwitch.isOn = false
        }
    }
    ...
    @objc func postBtnTapped(sender: UIButton){
        ...
        self.paramDic.updateValue("Jobs", forKey: "itemCategory")
        ...
        self.paramDic.updateValue(payPeriodTextField.text!, forKey: "payPeriod")
        if listIndefinitlySwitch.isOn == true{
            self.paramDic.updateValue("-1", forKey: "endTime")
        }else {
            self.paramDic.updateValue(listingDurationValue, forKey: "endTime")
        }
        self.paramDic.updateValue(automaticRelistingSwitch.isOn, forKey: "autoRelist")
        
        if priceTextField.text!.isEmpty || priceTextField.text! == "0" {
            showSwiftMessageWithParams(theme: .error, title: "strListing".localizableString(loc: LanguageChangeCode), body: "Please Enter Salary")
            priceTextField.becomeFirstResponder()
        }
        else if payPeriodTextField.text!.isEmpty ||  payPeriodTextField.text! == "Select Category"{
            showSwiftMessageWithParams(theme: .error, title: "strListing".localizableString(loc: LanguageChangeCode), body: "Please select Pay Period")
            payPeriodTextField.becomeFirstResponder()
        } else if  listIndefinitlySwitch.isOn == false && listingDurationTextField.text! == "Select Days"{
          ...
          listingDurationTextField.becomeFirstResponder()
        }
        else if listIndefinitlySwitch.isOn == false && listingDurationValue == "" {
            showSwiftMessageWithParams(theme: .error, title: "strListing".localizableString(loc: LanguageChangeCode), body: "Please select listing Duration")
            listingDurationTextField.becomeFirstResponder()
        }else {
            print(paramDic)
    ...
    private func setupView() {
        ...
        listIndefinitlySwitch.addTarget(self, action: #selector(listIndefinitelyBtnTapped(sender:)), for: .allTouchEvents)
        postBtn.addTarget(self, action: #selector(postBtnTapped(sender:)), for: .touchUpInside)
        selectCreatePicker(textField: payPeriodTextField, tag: 1)
        selectCreatePicker(textField: listingDurationTextField, tag: 2)
        createToolBar(textField: payPeriodTextField)
        createToolBar(textField: listingDurationTextField)
    ...
    private func postItem(body:[String:Any]){
      ...
      return showSwiftMessageWithParams(theme: .error, title: "strListing".localizableString(loc: LanguageChangeCode), body: "Please select a country!")
        ...
        Networking.instance.postApiCall(url: addItemUrl, param: body) { (response, Error, StatusCode) in
...
extension JobListingPostVC: UIPickerViewDataSource, UIPickerViewDelegate{
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        if pickerView == defaultCurrencyPicker {
           ...
           payPeriodTextField.text! = payPeriodArray[row]
        }else {
            listingDurationTextField.text! = durationArray[row] + " Days"
...
extension JobListingPostVC: UITextFieldDelegate{
