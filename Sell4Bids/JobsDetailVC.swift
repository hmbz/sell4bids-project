...
class JobsDetailVC: UIViewController,GMSMapViewDelegate,UIGestureRecognizerDelegate {
    ...
    @IBOutlet weak var viewApplicationBtn: UIButton!
  ...
  @IBOutlet weak var companyDescriptionLbl12: UILabel!
  ...
  @objc func methodOfReceivedNotification(notification: Notification) {
    print("here is")
    self.applyNowBtn.backgroundColor = UIColor.gray
               self.applyNowBtn.isEnabled = false
//               self.listedEndedLblHeight.constant = 0
//               self.listedEndedLbl.text = "Already Applied"
    self.isaplied = true
    ...
    private func setupDetailViews() {
        ...
        chatBtn.shadowView()
        viewApplicationBtn.shadowView()
        applyNowBtn.shadowView()
    ...
    private func showDataOnView() {
        ...
        self.categoryLbl.text = selectedProduct?.jobCategory
        ...
        self.employmentTypeLbl.text = selectedProduct?.employmentType
        self.benefitsLbl.text = "Medical"
        self.payPeriodLbl.text = selectedProduct?.payPeriod
        self.experienceLbl.text = selectedProduct?.jobExperience
      if selectedProduct?.companyName == ""{
        self.companyNameLbl.isHidden = true
      }else{
        self.companyNameLbl.text = selectedProduct?.companyName
      }
        
      if selectedProduct?.companyDescription == "(Optional)" || selectedProduct?.companyDescription == "" {
        self.companyDescriptionLbl12.isHidden = true
      }else{
        self.companyDescriptionLbl.text = selectedProduct?.companyDescription
      }
        ...
        if userId == self.selectedProduct?.uid {
            ...
            self.userButtonStackHeight.constant = 282
            ...
            self.viewApplicationBtn.isHidden = false
            ...
            self.listedByYouLblHeight.constant = 0
             ...
             self.OfferOrderViewHeight.constant = 0
             self.viewApplicationBtn.isHidden = true
      ...
      if isaplied == true{
        ...
        if selectedProduct?.jobApplied == true{
            self.applyNowBtn.backgroundColor = UIColor.lightGray
            self.applyNowBtn.isEnabled = false
            self.listedEndedLblHeight.constant = 25
            self.listedEndedLbl.text = "Already Applied"
        }
        else {
            self.applyNowBtn.backgroundColor = UIColor.black
            self.applyNowBtn.isEnabled = true
            self.listedEndedLblHeight.constant = 0
...
//        if timeRemaining2 == 0 || timeRemaining2 < -1  {
//            self.applyNowBtn.backgroundColor = UIColor.lightGray
//            self.applyNowBtn.isEnabled = false
//
//        }
...
//        }
    ...
    private func performActionOnButtons() {
        ...
        self.viewApplicationBtn.addTarget(self, action: #selector(viewApplicationBtnTapped(sender:)), for: .touchUpInside)
        self.applyNowBtn.addTarget(self, action: #selector(applyNowBtnTapped(sender:)), for: .touchUpInside)
        ...
        if SessionManager.shared.isUserLoggedIn == true {
            ...
            Networking.instance.postApiCall(url: itemDetailUrl, param: parameter) { (response, error, statusCode) in
                ...
                if status {
                    ...
                    let jobApplied = jsonDic["jobApplied"]?.bool ?? false
                    ...
                    for item in msgArray {
                        ...
                        // Data specific To Job
                        let jobExperience = msgDic["jobExperience"]?.string ?? ""
                        let companyDescription = msgDic["companyDescription"]?.string ?? ""
                        let companyName = msgDic["companyName"]?.string ?? ""
                        let employmentType = msgDic["employmentType"]?.string ?? ""
                        let payPeriod = msgDic["payPeriod"]?.string ?? ""
                        let jobCategory = msgDic["jobCategory"]?.string ?? ""
                        let benefitsArray = msgDic["benefits"]?.array ?? []
                        // Curency
                       ...
                       // Final Data
                        self.selectedProduct = jobsDetailModel.init(watchlistStatus: watchlistStatus, status: status, sellerIsBlocked: sellerIsBlocked, unauthorized: unauthorized,jobApplied: jobApplied, itemId: itemId, uid: uid, startPrice: startPrice, condition: condition, imagePath: imagePath, imagePathSmall: imagePathSmall, title: title, itemCategory: itemCategory, quantity: quantity, description: description, jobExperience: jobExperience, companyDescription: companyDescription, companyName: companyName, employmentType: employmentType, payPeriod: payPeriod, jobCategory: jobCategory, benefitsArray: benefitsArray, currencyString: currencyString, currenySymbol: currenySymbol, chargeTime: chargeTime, startTime: startTime, endTime: endTime, city: city, zipCode: zipCode, state: state, countryCode: countryCode, longitude: longitude, latitude: latitude, itemAuctionType: itemAuctionType, acceptOffers: acceptOffers, ordering: ordering, visibility: visibility, autoReList: autoReList, platform: platform, listingEnded: listingEnded, autoListingCount: autoListingCount, adminVerify: adminVerify, orderArray: self.orderArray, offerArray: self.offerArray)
...
//            let uid = self.selectedProduct?.uid ?? ""
    ...
    @objc func viewApplicationBtnTapped(sender: UIButton) {
        ...
        vc.itemId = selectedProduct?.itemId ?? ""
        vc.modalPresentationStyle = .fullScreen
        let navController = UINavigationController(rootViewController: vc)
        self.present(navController, animated: true, completion: nil)
    }
    
    @objc func applyNowBtnTapped(sender: UIButton){
        
        if SessionManager.shared.isUserLoggedIn == true {
            let vc = storyboard?.instantiateViewController(withIdentifier: "ApplyNowVC") as! ApplyNowVC
    ...
    @objc func Submit_Btn_Report() {
      if ReportItemTxt == ""{
        return  showSwiftMessageWithParams(theme: .info, title: "Sell4bids!!", body: "StrTechnicalIssue1".localizableString(loc: LanguageChangeCode))
      }
         let parameter = [
                ...
                if(ifYes == true){
                    ...
                    Networking.instance.postApiCall(url: acceptOfferStatusChangeUrl, param: parameter) { (response, Error, StatusCode) in
                        ...
                        if jsonStatus == true {
                            ...
                            showSwiftMessageWithParams(theme: .info, title: StrOfferStatus, body: "StrTechnicalIssue".localizableString(loc: LanguageChangeCode))
                        }
                    }
                }
            }
...
extension JobsDetailVC: UITextFieldDelegate{
...
extension JobsDetailVC : UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
...
extension JobsDetailVC: UIPickerViewDataSource, UIPickerViewDelegate{
