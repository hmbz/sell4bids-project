...
class VehicleListingPostVC: UIViewController {
    ...
    private func postItem(body:[String:Any]){
        ...
        Networking.instance.listingApiCall(url: addVehicleUrl, param: body, ImagesArray: self.imageArray) { (response, Error, StatusCode) in
...
extension VehicleListingPostVC: UIPickerViewDataSource, UIPickerViewDelegate{
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        ...
        if pickerView == defaultCurrencyPicker {
            var symbol = String()
            let country = CurrencyManager.currencyArray[row].country
            if country == "UNITED STATES OF AMERICA" || country == "INDIA" || country == "USA" || country == "IND"{
                ...
                self.buyNowStartingDurationTextField.placeholder = symbol
                self.paramDic.updateValue(currencySymbol ?? "", forKey: "currency_symbol")
                self.paramDic.updateValue(TwoDigitCode ?? "", forKey: "currency_string")
            }else {
                showSwiftMessageWithParams(theme: .info, title: appName, body: "strVehicleLocationError".localizableString(loc: LanguageChangeCode))
            }
        }
...
extension VehicleListingPostVC: UITextFieldDelegate{
