...
class ServiceViewOrders: UIViewController ,UITableViewDelegate , UITableViewDataSource, UIGestureRecognizerDelegate {
    ...
    func ReloadOrderData() {
        ...
        print("ServiceDetails == \(ServiceDetails!.id)")
        MainApi.Get_Orders(seller_uid: SessionManager.shared.userId, item_id: self.ServiceDetails!.id, type: "orders", sellerImage: "") { (status, data, error) in
