...
class VehicleDetailVC: UIViewController,GMSMapViewDelegate,UIGestureRecognizerDelegate {
    ...
    @IBOutlet weak var kmDrivenLbl: UILabel!
    ...
    var selectedProduct : vehicleDetailModel?
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.methodOfReceivedNotification(notification:)), name: Notification.Name("NotificationIdentifier12"), object: nil)
  ...
  @objc func methodOfReceivedNotification(notification: Notification) {
    ...
    self.chatBtn.backgroundColor = #colorLiteral(red: 0.6666666865, green: 0.6666666865, blue: 0.6666666865, alpha: 1)
...
//        let currencySymbol = CurrencyManager.instance.getCurrencySymbol(Country: selectedProduct?.countryCode ?? "USA")
        self.priceLbl.text = "\(selectedProduct?.currenySymbol ?? "") \(selectedProduct?.startPrice ?? 0)"
        ...
        self.yearLbl.text = "\(selectedProduct?.year ?? 0)"
        self.makeLbl.text = selectedProduct?.make
        self.modelLbl.text = selectedProduct?.model
        self.trimLbl.text = selectedProduct?.trim
        self.fuelTypeLbl.text  = selectedProduct?.fuelType
        self.kmDrivenLbl.text = "\(selectedProduct?.milesDriven ?? 0)"
        self.colorLbl.text = selectedProduct?.color
        
        let startTime:TimeInterval = Double(selectedProduct?.startTime ?? 0)
        ...
        if timeRemaining == 0 || timeRemaining < -1 && self.selectedProduct?.endTime != -1  {
            self.listedEndedLbl.text = "Listing Ended"
          ...
          //            self.ofNowBtn.isUserInteractionEnabled = true
          //            self.buyNowBtn.isUserInteractionEnabled = true
          //
          //            self.bidNowBtn.bferackgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1)
//            self.offerNowBtn.backgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1)
//            self.buyNowBtn.backgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1)
            ...
            Networking.instance.postApiCall(url: itemDetailUrl, param: parameter) { (response, error, statusCode) in
                ...
                if status {
                    ...
                    for item in msgArray {
                        ...
                        // Vehicle Data
                        let year = msgDic["year"]?.int ?? 0
                        let make = msgDic["make"]?.string ?? ""
                        let color = msgDic["color"]?.string ?? ""
                        let trim = msgDic["trim"]?.string ?? ""
                        let model = msgDic["model"]?.string ?? ""
                        let fuelType = msgDic["fuel_type"]?.string ?? ""
                        let milesDriven = msgDic["miles_driven"]?.int64 ?? 0
                        // Curency
    ...
    ShowSellerCounteOffer.acceptBtn.addTarget(self, action: #selector(acceptOfferOnCounterOfferBtnTapped(sender:)), for: .touchUpInside)
        ...
        let currencySymbol = selectedProduct?.currencyString ?? ""
    ...
    @objc func bidNowBtnTapped(sender: UIButton) {
        ...
        if SessionManager.shared.isUserLoggedIn == true {
            ...
            BidNowCustomView.sendBtn.setTitle("Placed Bid", for: .normal)
            let currenySymbol = selectedProduct?.currenySymbol ?? ""
            BidNowCustomView.textFieldTitleLbl.text =  "Bid price must be greater than \(currenySymbol)\(selectedProduct?.startPrice ?? -1)"
            BidNowCustomView.firstTextField.placeholder = currenySymbol
        ...
        if BidNowCustomView.firstTextField.text!.isEmpty {
            ...
            Networking.instance.postApiCall(url: bidItemUrl, param: parameter) { (response, Error, StatusCode) in
                ...
                if jsonStatus == true {
                    ...
                    self.BidNowCustomView.firstTextField.text = ""
                    self.reloadData()
    ...
    @objc func offerNowBtnTapped(sender: UIButton) {
        ...
        if SessionManager.shared.isUserLoggedIn == true {
            ...
            OfferNowCustomView.sendBtn.setTitle("Send Offer", for: .normal)
        ...
        if OfferNowCustomView.firstTextField.text!.isEmpty || price <= 0 {
            ...
            Networking.instance.postApiCall(url: buyerOfferUrl, param: parameter) { (response, Error, StatusCode) in
                ...
                if jsonStatus == true {
                    let message = jsonDic["message"]?.string ?? ""
                    showSwiftMessageWithParams(theme: .success, title: "StrUpdateQuantity".localizableString(loc: LanguageChangeCode), body: message)
                    self.OfferNowCustomView.firstTextField.text = ""
                    self.OfferNowCustomView.firstTextField.text = ""
                    self.reloadData()
...
//        let uid = self.selectedProduct?.uid ?? ""
    ...
    @objc func acceptOfferBtnTapped(sender: UIButton){
        ...
        if self.acceptOfferBtn.titleLabel?.text == StrStopAcceptingOffer {
            ...
            SweetAlert().showAlert(StrAcceptOffer, subTitle: "StrQAllowBuyerToSendOffer".localizableString(loc: LanguageChangeCode), style: AlertStyle.warning, buttonTitle:"StrYes".localizableString(loc: LanguageChangeCode), buttonColor:UIColor.black, otherButtonTitle: "StrNo".localizableString(loc: LanguageChangeCode), otherButtonColor: UIColor.black) { (ifYes) -> Void in
                if(ifYes == true){
...
extension VehicleDetailVC : UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == imageCollectionView{
            ...
            cell.userImage.sd_setImage(with: URL(string: encodeImage), placeholderImage: #imageLiteral(resourceName: "Vehicle"))
            ...
            let image = ImageArray[indexPath.row]
            let encodeImage = image.replacingOccurrences(of: " ", with: "%20")
            cell.userImage.sd_setImage(with: URL(string: encodeImage), placeholderImage: #imageLiteral(resourceName: "My Sell4Bids"))
            return cell
        }
...
extension VehicleDetailVC: UIPickerViewDataSource, UIPickerViewDelegate{
