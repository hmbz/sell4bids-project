...
class JobListingStep3VC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        TakeDurationAndOffersCustom.acceptOfferLbl.text = "Salary Negotiable"
        JobSalaryView.addShadow()
        
        DollarLabel.text = currency_String
        DollarLabel.text = currency_Symbol
        
        JobDurationAndOffersView.frame = TakeDurationAndOffersCustom.frame
        JobDurationAndOffersView.addShadow()
        JobDurationAndOffersView.addSubview(TakeDurationAndOffersCustom)
        
        DollarView.layer.borderColor = UIColor.black.cgColor
        DollarView.layer.borderWidth = 2.0
        DollarView.makeCornersRound()
        Currency = ["PKR","USD","EUR","INR","BST"]
    ...
    func createCurrencyPicker() {
        ...
        //        categoriesTextField.inputView = Picker.Category
...
extension  JobListingStep3VC : UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource{
    ...
    func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing view: UIView?) -> UIView {
        ...
        if (pickerView == defaultCurrencyPicker) {
            let Array = CurrencyManager.currencyArray
            label.text = Array[row].country
            self.currency_String = Array[row].country ?? ""
        }else if (pickerView.tag == 3) {
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        if textField == JobSalaryTextField{
            if textField.text!.isEmpty{
                textField.placeholder = currency_Symbol
            }
