...
class ItemDetailVC: UIViewController,GMSMapViewDelegate,UIGestureRecognizerDelegate {
    ...
    @IBOutlet weak var viewBidsBtn: UIButton!
    ...
    @IBOutlet weak var setQuantityBtn: UIButton!
    ...
    @IBOutlet weak var quantityLblHeight: NSLayoutConstraint!
    ...
    var selectedProduct : itemDetailModel?
    ...
    let OfferNowCustomView = Bundle.main.loadNibNamed("MultipleTextFieldView", owner: self, options: nil)?.first as! MultipleTextFieldView
    //    Custom View For Bid Now functionality
    let bidNowDialogue = UIAlertController.init(title: "", message: "", preferredStyle: .alert)
    let BidNowCustomView = Bundle.main.loadNibNamed("singleTextFieldView", owner: self, options: nil)?.first as! singleTextFieldView
    // Buyy now Button Functionality
    let buyNowDialogue = UIAlertController.init(title: "", message: "", preferredStyle: .alert)
    let buyNowCustomView = Bundle.main.loadNibNamed("singleTextFieldView", owner: self, options: nil)?.first as! singleTextFieldView
    ...
    // Reject Offer View
    let RejectOfferView = Bundle.main.loadNibNamed("rejectCustomView", owner: self, options: nil)?.first as! rejectCustomView
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if touch?.view == OfferNowCustomView {
    ...
    private func setupDetailViews() {
        ...
        quantityLbl.layer.shadowColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1)
        quantityLbl.layer.shadowRadius = 2.0
        quantityLbl.layer.shadowOpacity = 1.0
        quantityLbl.layer.shadowOffset = CGSize(width: 0, height: 1)
        quantityLbl.layer.masksToBounds = false
    ...
    private func showDataOnView() {
        ...
        self.categoryLbl.text = selectedProduct?.itemCategory
        ...
        if userId == self.selectedProduct?.uid {
            ...
            if selectedProduct?.itemAuctionType == "buy-it-now" {
                ...
                self.userButtonStackHeight.constant = 339
                self.acceptOfferBtn.isHidden = false
                self.newOrderBtn.isHidden = false
                self.setQuantityBtn.isHidden = false
                ...
                self.viewBidsBtn.isHidden = true
                
                self.quantityLblHeight.constant = 18
                ...
                self.userButtonStackHeight.constant = 90
                self.acceptOfferBtn.isHidden = true
                self.newOrderBtn.isHidden = true
                self.setQuantityBtn.isHidden = true
                self.showHideItemBtn.isHidden = false
                self.automaticRelistBtn.isHidden = true
                self.reListItemBtn.isHidden = true
                self.endListingBtn.isHidden = true
                self.turboChargeBtn.isHidden = false
                self.listedEndedLbl.text = "Auction Ended"
                self.quantityLblHeight.constant = 0
                self.OfferOrderViewHeight.constant = 40
                self.viewOfferBtn.isHidden = true
                self.viewOrderBtn.isHidden = true
                self.viewBidsBtn.isHidden = false
            }
        }
        else {
        ...
        if selectedProduct?.itemAuctionType == "buy-it-now" {
            // che ckecking for simple Listing
            // setting up the colors of the Buttons
            if timeRemaining == 0 || timeRemaining < -1  {
                self.bidNowBtn.isUserInteractionEnabled = false
                self.offerNowBtn.isUserInteractionEnabled = false
                self.buyNowBtn.isUserInteractionEnabled = false
                self.bidNowBtn.backgroundColor = #colorLiteral(red: 0.6666666865, green: 0.6666666865, blue: 0.6666666865, alpha: 1)
                self.offerNowBtn.backgroundColor = #colorLiteral(red: 0.6666666865, green: 0.6666666865, blue: 0.6666666865, alpha: 1)
                self.buyNowBtn.backgroundColor = #colorLiteral(red: 0.6666666865, green: 0.6666666865, blue: 0.6666666865, alpha: 1)
                ...
                self.bidNowBtn.isUserInteractionEnabled = true
                self.offerNowBtn.isUserInteractionEnabled = true
                self.buyNowBtn.isUserInteractionEnabled = true
                
                self.bidNowBtn.backgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1)
                self.offerNowBtn.backgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1)
                self.buyNowBtn.backgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1)
    ...
    private func performActionOnButtons() {
        ...
        self.bidNowBtn.addTarget(self, action: #selector(bidNowBtnTapped(sender:)), for: .touchUpInside)
        ...
        self.viewBidsBtn.addTarget(self, action: #selector(viewBidsBtnTapped(sender:)), for: .touchUpInside)
        ...
        self.setQuantityBtn.addTarget(self, action: #selector(setQuantityBtnTapped(sender:)), for: .touchUpInside)
        ...
        if SessionManager.shared.isUserLoggedIn {
            ...
            Networking.instance.postApiCall(url: itemDetailUrl, param: parameter) { (response, error, statusCode) in
                ...
                if status {
                    ...
                    let sellerIsBlocked = jsonDic["sellerIsBlocked"]?.bool ?? false
                    let unauthorized = jsonDic["unauthorized"]?.bool ?? false
                    let msgDic = jsonDic["message"]?.dictionary
                    //  let msgDic = item.dictionary
                    // ID's Used in the Data
                    let itemId = msgDic?["_id"]?.string ?? ""
                    let uid = msgDic?["uid"]?.string ?? ""
    ...
    // Bid Now Api call
    @objc func bidNowBtnTapped(sender: UIButton) {
        print("View Bids Button Tapped")
        
        if SessionManager.shared.isUserLoggedIn == true {
            BidNowCustomView.titleLblHeight.constant = 0
            BidNowCustomView.appIconHeight.constant = 0
            BidNowCustomView.firstTextField.tag = 1
            BidNowCustomView.firstTextField.delegate = self
            BidNowCustomView.firstTextField.keyboardType = .numberPad
            BidNowCustomView.sendBtn.setTitle("Place Bid", for: .normal)
            let currencySymbol = selectedProduct?.currenySymbol ?? "Price"
            BidNowCustomView.textFieldTitleLbl.text =  "Bid price must be greater than \(currencySymbol)\(selectedProduct?.startPrice ?? -1)"
            BidNowCustomView.firstTextField.placeholder = currencySymbol
            BidNowCustomView.crossBtn.addTarget(self, action: #selector(BidNowCloseBtnTapped), for: .touchUpInside)
            bidNowDialogue.view.frame = BidNowCustomView.frame
            bidNowDialogue.view.addSubview(BidNowCustomView)
            BidNowCustomView.sendBtn.addTarget(self, action: #selector(bidNowApiCall), for: .touchUpInside)
            self.present(bidNowDialogue, animated: true, completion: nil)
        }else {
            SweetAlert().showAlert("strLoginRequired".localizableString(loc: LanguageChangeCode), subTitle: "strLoginRequiredForListing".localizableString(loc: LanguageChangeCode), style: AlertStyle.warning, buttonTitle: "StrLogIn".localizableString(loc: LanguageChangeCode), buttonColor: UIColor.black, otherButtonTitle: "strCancel".localizableString(loc: LanguageChangeCode), otherButtonColor: UIColor.black) { (ifYes) -> Void in
                if(ifYes == true){
                    let SB = UIStoryboard(name: "Main", bundle: nil)
    ...
    @objc func BidNowCloseBtnTapped() {
      
        bidNowDialogue.dismiss(animated: true, completion: nil)
    }
    
    
    @objc func bidNowApiCall() {
        let price = Int(BidNowCustomView.firstTextField.text!)
        let startPrice = selectedProduct?.startPrice ?? 0
        if price ?? 0 < startPrice || price == startPrice{
            self.view.makeToast("strBidNowValidation".localizableString(loc: LanguageChangeCode), position:.top)
        }else {
            
            let parameter = [
                "item_id" : selectedProduct?.itemId ?? ""
                ,"bidAmount": BidNowCustomView.firstTextField.text!
                ,"buyerUid" : SessionManager.shared.userId
                ,"buyerName" : SessionManager.shared.name
                ,"buyerToken" : SessionManager.shared.fcmToken
                ,"buyerEmail" : SessionManager.shared.email
            ]
            
            Networking.instance.postApiCall(url: bidItemUrl, param: parameter) { (response, Error, StatusCode) in
                guard let jsonDic = response.dictionary else {return}
                let jsonStatus = jsonDic["status"]?.bool ?? false
                let message = jsonDic["message"]?.string ?? ""
                if jsonStatus == true {
                    showSwiftMessageWithParams(theme: .success, title: "StrBids".localizableString(loc: LanguageChangeCode), body: message)
                    self.bidNowDialogue.dismiss(animated: true, completion: nil)
                    self.BidNowCustomView.firstTextField.text = nil
                    self.reloadData()
                }else {
                  showSwiftMessageWithParams(theme: .info, title: "StrUpdateQuantity".localizableString(loc: LanguageChangeCode), body: message)
                }
            }
        }
    }
    ...
    @objc func offerNowBtnTapped(sender: UIButton) {
        ...
        if SessionManager.shared.isUserLoggedIn == true {
            let currencySymbol = selectedProduct?.currenySymbol
            OfferNowCustomView.priceTextField.keyboardType = .numberPad
            OfferNowCustomView.priceTextField.delegate = self
            OfferNowCustomView.priceTextField.placeholder = currencySymbol
            OfferNowCustomView.quantityTextField.keyboardType = .numberPad
            OfferNowCustomView.quantityTextField.delegate = self
    ...
    @objc func offerNowApiCall() {
        let quantity = selectedProduct?.quantity ?? 0
        let price = Int(OfferNowCustomView.priceTextField.text!) ?? 0
        let quantityText = Int(OfferNowCustomView.quantityTextField.text!) ?? 0
        if OfferNowCustomView.priceTextField.text!.isEmpty || price <= 0 {
            self.view.makeToast(strEmptyPriceField, position: .top)
        }else if quantityText == 0 || quantityText > quantity {
            self.view.makeToast("strQuantityAccordingToStock".localizableString(loc: LanguageChangeCode), position: .top)
        }
        else {
            let parameter:[String:Any] = [
                "buyer_uid": SessionManager.shared.userId,
                "offer_price" : OfferNowCustomView.priceTextField.text!,
                "item_id" : selectedProduct?.itemId ?? "",
                "offerType" : "offers",
                "buyer_name" : SessionManager.shared.name,
                "buyer_image" : SessionManager.shared.image,
                "offer_quantity" : quantityText,
            ...
            Networking.instance.postApiCall(url: buyerOfferUrl, param: parameter) { (response, Error, StatusCode) in
                ...
                if jsonStatus == true {
                    let message = jsonDic["message"]?.string ?? ""
                    showSwiftMessageWithParams(theme: .success, title: StrOffers, body: message)
                    self.OfferNowCustomView.priceTextField.text = ""
                    self.OfferNowCustomView.quantityTextField.text = ""
                    self.reloadData()
    ...
    @objc func buyNowBtnTapped(sender: UIButton) {
        ...
        if SessionManager.shared.isUserLoggedIn == true {
            buyNowCustomView.titleLblHeight.constant = 30
            buyNowCustomView.appIconHeight.constant = 30
            buyNowCustomView.sendBtn.setTitle("Send Order", for: .normal)
            buyNowCustomView.textFieldTitleLbl.text =  "Quantity to buy"
            buyNowCustomView.firstTextField.keyboardType = .numberPad
          
            buyNowCustomView.firstTextField.delegate = self
            buyNowCustomView.crossBtn.addTarget(self, action: #selector(buyNowCloseBtn), for: .touchUpInside)
            buyNowDialogue.view.frame = buyNowCustomView.frame
            buyNowDialogue.view.addSubview(buyNowCustomView)
            buyNowCustomView.sendBtn.addTarget(self, action: #selector(buyNowApiCall), for: .touchUpInside)
            self.present(buyNowDialogue, animated: true, completion: nil)
        }else {
            SweetAlert().showAlert("strLoginRequired".localizableString(loc: LanguageChangeCode), subTitle: "strLoginRequiredForListing".localizableString(loc: LanguageChangeCode), style: AlertStyle.warning, buttonTitle: "StrLogIn".localizableString(loc: LanguageChangeCode), buttonColor: UIColor.black, otherButtonTitle: "strCancel".localizableString(loc: LanguageChangeCode), otherButtonColor: UIColor.black) { (ifYes) -> Void in
                if(ifYes == true){
                    let SB = UIStoryboard(name: "Main", bundle: nil)
    ...
    @objc func buyNowCloseBtn(sender: UIButton) {
        buyNowDialogue.dismiss(animated: true, completion: nil)
    }
    
    @objc func buyNowApiCall() {
        let quantity = selectedProduct?.quantity ?? 0
        let BuyNowQuantity = Int(buyNowCustomView.firstTextField.text!) ?? 0
        if buyNowCustomView.firstTextField.text!.isEmpty{
            self.view.makeToast("strEmptyQuantity".localizableString(loc: LanguageChangeCode), position: .top)
        } else if buyNowCustomView.firstTextField.text! == "0"{
            self.view.makeToast("zeroQuantity".localizableString(loc: LanguageChangeCode), position: .top)
        }
        else if BuyNowQuantity > quantity{
            self.view.makeToast("strQuantityAccordingToStock".localizableString(loc: LanguageChangeCode), position: .top)
        }
        else {
            let parameter:[String:Any] = [
                "bought_quantity" : BuyNowQuantity,
                "buyer_name" : SessionManager.shared.name,
            ...
            Networking.instance.postApiCall(url: buyItemUrl, param: parameter) { (response, Error, StatusCode) in
                ...
                if jsonStatus == true {
                    ...
                    self.buyNowCustomView.firstTextField.text = ""
                    self.reloadData()
                    self.buyNowDialogue.dismiss(animated: true, completion: nil)
                }else {
                    showSwiftMessageWithParams(theme: .info, title: "StrUpdateQuantity".localizableString(loc: LanguageChangeCode), body: "StrTechnicalIssue".localizableString(loc: LanguageChangeCode))
                }
            }
    ...
    // Set Quantity Button Tapped
    @objc func setQuantityBtnTapped(sender: UIButton){
        print("Set Quantity Button Tapped")
        
        setItemQuantityCustomView.Quantitytxt.delegate = self
        setItemQuanityDialogue.view.frame = setItemQuantityCustomView.frame
        setItemQuantityCustomView.CloseBtn.addTarget(self, action: #selector(setQuantityCloseBtnTapped), for: .touchUpInside)
        setItemQuantityCustomView.SendOfferBtn.addTarget(self, action: #selector(setQuantityApiCall), for: .touchUpInside)
        setItemQuantityCustomView.currency_constraint.constant = 0
        setItemQuantityCustomView.Heading_Title.text = "Stock Quantity"
        setItemQuantityCustomView.SendOfferBtn.setTitle("Set Quantity", for: .normal)
        setItemQuantityCustomView.SendOfferBtn.addShadowAndRound()
        setItemQuanityDialogue.view.addSubview(setItemQuantityCustomView)
        self.present(setItemQuanityDialogue, animated: true, completion: nil)
        setItemQuantityCustomView.Quantitytxt.placeholder = "Enter your quantity"
    }
    
    @objc func setQuantityCloseBtnTapped() {
        setItemQuanityDialogue.dismiss(animated: true, completion: nil)
    }
    
    @objc func setQuantityApiCall() {
        print("Update Item Quantity Button Tapped")
        if setItemQuantityCustomView.Quantitytxt.text!.isEmpty{
            showSwiftMessageWithParams(theme: .info, title: "StrUpdateQuantity".localizableString(loc: LanguageChangeCode), body: StrUpodateEmptyQuantity)
        }else {
            let checkQuantity = Int(setItemQuantityCustomView.Quantitytxt.text!)!
            let parameter:[String:Any] = ["quantity" : checkQuantity, "item_id" : self.selectedProduct?.itemId ?? ""]
            Networking.instance.postApiCall(url: updateQuantityUrl, param: parameter) { (response, Error, StatusCode) in
                guard let jsonDic = response.dictionary else {return}
                let jsonStatus = jsonDic["status"]?.bool ?? false
                if jsonStatus == true {
                    let message = jsonDic["message"]?.string ?? ""
                    showSwiftMessageWithParams(theme: .success, title: "StrUpdateQuantity".localizableString(loc: LanguageChangeCode), body: message)
                    self.setItemQuanityDialogue.dismiss(animated: true, completion: nil)
                    self.setItemQuantityCustomView.Quantitytxt.text = ""
                    self.reloadData()
                }else {
                    showSwiftMessageWithParams(theme: .info, title: "StrUpdateQuantity".localizableString(loc: LanguageChangeCode), body: "StrTechnicalIssue".localizableString(loc: LanguageChangeCode))
                }
    ...
    @objc func reListItemBtnTapped(sender: UIButton){
        ...
        let symbol = selectedProduct?.currenySymbol ?? ""
    ...
    @objc func endListingBtnTapped(sender: UIButton){
        ...
        endListingCustomView.shadowView()
        endListingCustomView.layer.cornerRadius = 6
    ...
    @objc func rejectOfferBuyerBtnTapped(sender: UIButton){
        ...
        RejectOfferView.continueBtn.addTarget(self, action: #selector(submitReasonTappedonCounterOffer), for: .touchUpInside)
        RejectOfferView.cancelBtn.addTarget(self, action: #selector(closeRejectReason), for: .touchUpInside)
        self.present(RejectOfferViewAlert, animated: true, completion: nil)
    }
    
    @objc func submitReasonTappedonCounterOffer(sender: UIButton){
        if RejectOfferView.selectedBtnTitle == "" {
            ...
            let body:[String: Any] = [
                ...
                "orderRejectReason": RejectOfferView.selectedBtnTitle
            ]
            print(body)
    ...
    @objc func sendCounterOfferTapped(sender: UIButton){
        ...
        if SendCounterOffer.AmountPerItemtxt.text == "" {
            ...
            let body:[String:Any] = [
                ...
                "offer_quantity": self.SendCounterOffer.Quantitytxt.text ?? "1",
...
extension ItemDetailVC: UITextFieldDelegate{
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if  textField == BidNowCustomView.firstTextField || textField == OfferNowCustomView.priceTextField{
            let allowedCharacters = "1234567890"
            let allowedCharacterSet = CharacterSet(charactersIn: allowedCharacters)
            let typedCharacterSet = CharacterSet(charactersIn: string)
            let alphabet = allowedCharacterSet.isSuperset(of: typedCharacterSet)
            return alphabet
        }else if textField == OfferNowCustomView.quantityTextField || textField == buyNowCustomView.firstTextField{
            let allowedCharacters = "1234567890"
            let allowedCharacterSet = CharacterSet(charactersIn: allowedCharacters)
            let typedCharacterSet = CharacterSet(charactersIn: string)
            let alphabet = allowedCharacterSet.isSuperset(of: typedCharacterSet)
...
extension ItemDetailVC: UIPickerViewDataSource, UIPickerViewDelegate{
