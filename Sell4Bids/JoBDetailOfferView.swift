...
class JoBDetailOfferView: UIViewController ,UITableViewDelegate , UITableViewDataSource, UIGestureRecognizerDelegate {
    ...
    @objc func Rejected_Order_Api() {
        ...
        MainApi.Reject_Order_Api(seller_uid: SessionManager.shared.userId, itemId: Rejecteditemid, orderId: RejectedOrderid, sellerName: SessionManager.shared.name, sellerImage: "",orderRejectReason: RejectedReason, quantity: selectedProduct!.quantity) { (status, data, error) in
            print("status == \(status)")
            print("data == \(data)")
            print("error == \(error)")
    ...
    override func viewDidLoad() {
        ...
        MainApi.Get_Orders(seller_uid: SessionManager.shared.userId, item_id: selectedProduct!.id, type: "orders", sellerImage: SessionManager.shared.image , start: "0" , limit: "25") { (status, data, error) in
            ...
            if status {
                ...
                for msg in message {
                    ...
                    print("name ===== \(username)")
                    let orders = OrderViewModel.init(item_id: item_id, order_status: status, image: image, user_id: usrid, user_averagerating: avgratting, name: username, order_id: orderid , type: type, buyer_item_recieved: buyer_item_recieved, price: boughtprice, seller_marked_paid: seller_marked_paid, quantity: boughtQuantity , isLocationShared: isLocationShared , address: address, seller_rating: seller_rating, averagerating: 0.1, orderRejectReason: "")
